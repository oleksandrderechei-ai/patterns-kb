...
${list(cmdRows)}
  ...
  <!-- Deferred: the catalog is 280KB and only the "elsewhere in the KB" results need it,
       so the term filter works from the moment the page parses and the corpus results
       light up when it lands. search.js exposes window.KB_MATCHES at module scope; its
       own UI never mounts here because there is no .controls element. -->
