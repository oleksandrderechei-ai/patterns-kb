...
<head>
  ...
  <!-- Synchronous, like theme.js: it stamps the stored mode on <html> before first paint,
       so a reader who left in A-Z does not watch the grouped view reflow away. -->
...
${list(langRows)}
    ...
    <section class="doc-section vocab-section" id="cli" aria-labelledby="h-cli">
      ...
      <div class="prose">
        <p>The one vocabulary here that is not page metadata: <code>scripts/kb.mjs</code>, which is how anything reads or writes this knowledge base without opening the HTML. The corpus is about 490k tokens and half of any page is markup, so a grounded answer costs a couple of thousand tokens through these commands instead of hundreds of thousands.</p>
...
${list(cmdRows)}
  ...
  <!-- Deferred: the catalog is 280KB, and two things here want it — the "elsewhere in the KB"
       results and the ⌘K palette. Deferring means the term filter works from the moment the
       page parses and both light up when the catalog lands. search.js exposes
       window.KB_MATCHES at module scope; its own UI never mounts here because there is no
       .controls element, which is also what lets palette.js own the chord. This order is the
       contract — see assets/palette.js. -->
